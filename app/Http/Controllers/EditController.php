...
class EditController extends Controller
{
    ...
    public function index($sql_error = false, $sql_success = false, $page_col_selected = array(), $page_selected_id = '')
    {
        ...
        $supported_tables = array('school', 'class');

        // regardless of new or old, fill out name, is_list, value, and display
        EditController::generate($page_selected_table, $page_col_name, $page_col_is_list, $page_id_name);
        ...
        // now shwo the displayed values
        // if we are coming from an edit, maintain $page_col_selected
        if(count($page_col_selected) == count($page_col_name)) {
            // maintain it.
        }
        ...
        return view('dashboard.edit_entry', compact('page_title', 'sql_error', 'sql_success',
        ...
        try {
            ...
            if($page_selected_id == '0') {
                ...
                if($page_selected_table == 'system_users') {
                    ...
                    $update_array['created_at'] = new DateTime();
                }
    ...
    private function generate($table_name, &$page_col_name, &$page_col_is_list, &$page_id_name) {
        if($table_name == 'school') {
            $page_col_name = array('school_name', 'school_address', 'weekend_days');
            $page_col_is_list = array(false, false, true);
            $page_id_name = 'school_id';

        } else if($table_name == 'class') {
            $page_col_name = array('school_id', 'grade', 'section', 'class_name');
            $page_col_is_list = array(true, false, false, false);
            $page_id_name = 'class_id';
