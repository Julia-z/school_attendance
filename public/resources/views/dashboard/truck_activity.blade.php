...
<div class="row gutter-xs">
    <div class="col-md-6">
        <div class="card">
          <div class="card-body">
            <h4 class="card-title">Total Trucks Inside Camp by Time (Average)</h4>
          </div>
          <div class="card-body">
            <div class="card-chart">
              ...
              @for($i = 13; $i <= 36; $i++)
                  ,
                  @if($i < 24 && $i%2 == 0) "{{floor($i/2)}}:00 am" @endif
                  @if($i < 24 && $i%2 != 0) "{{floor($i/2)}}:30 am" @endif
                  @if($i == 24) "12:00 pm" @endif
                  @if($i == 25) "12:30 pm" @endif
                  @if($i > 25 && $i%2 == 0) "{{ floor($i/2) - 12 }}:00 pm" @endif
                  @if($i > 25 && $i%2 != 0) "{{ floor($i/2) - 12 }}:30 pm" @endif
              @endfor ]'
              data-values='[{"label": "Trucks", "backgroundColor": "transparent", "borderColor": "#0ac29d", "pointBackgroundColor": "#0ac29d", "data": [ {{ number_format($count_by_hour[12], 2) }}
              @for($i = 13; $i <= $graph_max_time; $i++) ,{{ number_format($count_by_hour[$i], 2) }} @endfor ]}]'
              data-tooltips='{"mode": "label"}' data-hide='["gridLinesX", "legend"]' height="100"></canvas>
            </div>
          </div>
        </div>
    </div>
    ...
    <div class="col-md-6">
        <div class="card">
          <div class="card-body">
            <h4 class="card-title">Truck Attendance by Day</h4>
          </div>
          <div class="card-body">
            <div class="card-chart">
              ...
              data-values='[{"label": "Trucks", "backgroundColor": "transparent", "borderColor": "#0ac29d", "pointBackgroundColor": "#0ac29d", "data": [ {{ $count_by_day[0] }}
    ...
    <div class="col-md-12">
        <div class="card">
          <div class="card-body">
            <h4 class="card-title">Desludging Capacity (hr x m<sup>3</sup>) vs Date</h4>
          </div>
          <div class="card-body">
            <div class="card-chart">
              <canvas data-chart="line" data-animation="false" data-labels='[ "{{ $count_by_day_x_axis[0] }}"
              @for($i = 1; $i < count($count_by_day_x_axis); $i++) ,"{{ $count_by_day_x_axis[$i] }}" @endfor ]'
              data-values='[{"label": "hours . m3", "backgroundColor": "transparent", "borderColor": "#0ac29d", "pointBackgroundColor": "#0ac29d", "data": [ {{ $capacity_by_day[0] }}
              @for($i = 1; $i < count($capacity_by_day); $i++) ,{{ $capacity_by_day[$i] }} @endfor ]}]'
              data-tooltips='{"mode": "label"}' data-hide='["gridLinesX", "legend"]' height="100"></canvas>
            </div>
          </div>
        </div>
...
<div class="row gutter-xs">
    <div class="col-xs-12 col-md-6">
      <div class="panel panel-body" data-toggle="match-height">
        <h4 class="text-center m-t-0">Average Attendance Distribution (Hover for Numbers)</h4>
        <div class="row">
            <div class="col-xs-3">
      ...
      <div class="card">
        <div class="card-header">
          <div class="card-actions">
           ...
           Truck List <span id="user_list_id"></span>
        </div>
            ...
            <tbody>
                @for($avg = 0; $avg < 7; $avg++)
                    ...
                    <tr class="names_for_{{ $avg }}" style="display: none;">
                        <td class="col-xs-6"> {{ $driver->name }} (Capacity: {{$driver->capacity}}m<sup>3</sup>)</td>
